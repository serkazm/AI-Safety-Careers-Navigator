...
async function handleFile(file) {
  ...
  try {
    ...
    state.cvCategory = data.category;
...
/* ─────────────────────────────────────────
   Step 2 – Questions
───────────────────────────────────────── */
async function showQuestions(classification) {
  // Fetch questions
  const res = await fetch('/api/questions');
  const { questions } = await res.json();
  state.questions = questions;

  // Populate classification box
  $('classification-badge').textContent = `🎯 ${classification.category}`;
  $('classification-reasoning').textContent = classification.reasoning;

  const factorsList = $('classification-factors');
  factorsList.innerHTML = '';
  (classification.key_factors || []).forEach(f => {
    const li = document.createElement('li');
    li.textContent = f;
    factorsList.appendChild(li);
  });
  $('classification-box').classList.remove('hidden');
...
const LOADING_CAPTIONS = [
  "Crunching your career trajectory through our highly sophisticated recommendation engine...",
  "Cross-referencing your skills with the AI safety universe...",
  "Consulting the alignment oracle...",
  "Almost there — polishing your personalised roadmap...",
  "Scanning every corner of the AI safety landscape for you...",
  "Connecting the dots between your experience and your future...",
];
